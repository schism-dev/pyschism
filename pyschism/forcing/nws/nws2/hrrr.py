...
class HRRRInventory:
    ...
    def check_dates(self, start_date, rnday):
        ...
        while nc is None:
            dt = nearest_cycle(
                nearest_cycle(datetime.utcnow()) - (i * self.file_interval)
            )
            cycle = dt.hour % 24
            base_url = (
                BASE_URL
    ...
    def put_sflux_field(self, hrrr_varname: str, dst: Dataset, sflux_varname: str):
    ...
    def _modified_bbox(self, bbox=None):
        if bbox is None:
            return Bbox.from_extents(
                np.min(self.lon), np.min(self.lat), np.max(self.lon), np.max(self.lat)
            )
        return bbox
        ...
        if air is True:
            with Dataset(
                ...
                for var in AirComponent.var_types:
                    ...
                    self.inventory.put_sflux_field(
