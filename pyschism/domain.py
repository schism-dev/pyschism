...
class OpenBoundariesDescriptor:
    ...
    def __get__(self, obj, val):
        ...
        if open_boudaries is None:
            open_boudaries = {}
            for id, bnd in obj.hgrid.open_boundaries.items():
                indexes = list(map(
                    obj.hgrid.get_vertex_index_by_id, bnd.indexes))
                open_boudaries[id] = {
                    'indexes': indexes, 'forcing': None}
...
class IcsDescriptor:
    ...
    def __get__(self, obj, val):
        if obj.hgrid.crs is None:
            return None
        elif obj.hgrid.crs.is_geographic:
            return 2
        else:
            return 1
